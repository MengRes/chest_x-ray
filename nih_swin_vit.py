...
logging.basicConfig(filename="logs/nih_swin_vit.log", level=logging.INFO,
...
class SwinViTModel(nn.Module):
    def __init__(self, num_classes):
        super(SwinViTModel, self).__init__()
...
model = SwinViTModel(num_classes=14)
...
for epoch in range(num_epochs):
    ...
    if val_auc > best_auc:
        best_auc = val_auc
        torch.save(model.state_dict(), "checkpoints/nih_swin_vit_best.pth")
...
# ========== Test ==========
model.load_state_dict(torch.load("checkpoints/nih_swin_vit_best.pth", weights_only = True))
